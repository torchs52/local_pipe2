...
| 種別 | 方針 |
|---|---|
| CE006 センサ校正データ不正 | 基本健全性に加え、参照行列との並進・回転・最大XY変位差分と生成直後検証を実装済み |
| CE007～CE010 カメラ校正データ不正 | fisheye JSONと外部行列の基本健全性を共通診断化。理想差分はstub、camera 3 path契約は未定義として明示発報 |
...
- CE007～CE010の理想差分仕様とcamera 3の正式な校正path契約
- 3D-3D参照行列の機種別許容閾値の製品承認
